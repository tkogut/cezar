...
3. **UI verification (conditional)** — if any Step in the window touched UI (pages, components, widgets, navigation):
   ...
   - **Post the checkpoint's verification outcome + screenshots to the PR.** The PR exists from step 7, so post immediately (never defer to the run folder alone): an idempotent verification comment — marker `` 🤖 `om-auto-create-pr-loop` — checkpoint <N> verification ``, naming the Step range, what the checks proved, pass/fail/skip with material limits, and a link to the checkpoint record; aim for 40–100 words, omit raw logs and repeated scope — and, when a Step in the window touched UI, this checkpoint's screenshots via **attach-image-evidence** — marker `` 🤖 `om-auto-create-pr-loop` — checkpoint <N> evidence ``, one line naming the Step range + touched areas, one line per image, slug `checkpoint-<N>`. Both are idempotent by marker: a re-run updates in place. Inline rendering is the tracker descriptor's job; when it cannot, the comment carries links — surface that, don't fail.
