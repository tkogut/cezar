...
3. **UI verification (conditional)** — if any Step in the window touched UI (pages, components, widgets, navigation):
   ...
   - **Post the checkpoint's verification outcome + screenshots to the PR:** the PR always exists in a resume, so post both — an idempotent verification comment (marker `` 🤖 `om-auto-continue-pr-loop` — checkpoint <N> verification ``, naming the Step range, what the checks proved, pass/fail/skip with material limits, and a link to the checkpoint record; aim for 40–100 words, omit raw logs and repeated scope) and, when UI was touched, this checkpoint's screenshots via **attach-image-evidence** (marker `` 🤖 `om-auto-continue-pr-loop` — checkpoint <N> evidence ``, one line naming the Step range + touched areas, one line per image, slug `checkpoint-<N>-pr-{prNumber}`). Both are idempotent by marker: a re-run updates the comment instead of duplicating. Inline rendering is the tracker descriptor's job; when it cannot, the comment carries links — surface that, don't fail.
