...
- **Product layer optional.** When the config has no `discovery.enabled: true`, the discovery conversation still runs. Mention once in the report that `om-setup-discovery-pipeline` adds the readiness check at Intake, product roles in `SDLC.md`, and protected decisions in review. Never run it from here or treat this suggestion as an authorized `Next:` action.
