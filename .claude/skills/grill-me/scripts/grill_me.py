#!/usr/bin/env python3
"""
🛸 AGENTS-OS v6.5 — Grill-Me Architectural Pre-Flight Protocol (Python Fallback)
"""
...
print(f"{CYAN}{BOLD}🛸 [AGENTS-OS v6.5] INICJALIZACJA PROTOKOŁU /grill-me{RESET}")
print(f"{YELLOW}Protokół Rygoru Architektonicznego przed Fazą Implementacji (Builder){RESET}")
print(f"{MAGENTA}{BOLD}{'=' * 68}{RESET}\n")
...
for q in questions:
    print(f"{GREEN}{BOLD}[Pytanie {q['id']}] {q['topic']}:{RESET}")
    print(f"  ❓ {BOLD}{q['question']}{RESET}")
    print(f"  💡 {YELLOW}Wytyczna:{RESET} {q['prompt']}\n")

print(f"{CYAN}{'━' * 60}{RESET}")
print(f"{YELLOW}{BOLD}📋 INSTRUKCJA DLA OPERATORA / KOORDYNATORA:{RESET}")
print(f"1. Przed przystąpieniem do kodowania wpisz odpowiedzi w {BOLD}.agents/MEMORY.md{RESET} (sekcja Decisions).")
print(f"2. Upewnij się, że zadania są rozpisane w {BOLD}task.md{RESET} z przypisanymi kryteriami akceptacji.")
print(f"3. Po zakończeniu wygeneruj {BOLD}_handshake.json{RESET} dla roli Auditora.")
print(f"{CYAN}{'━' * 60}{RESET}\n")
